...
export default function GallerySlide({isEgg, slide, bgSrc }) {
  ...
  return (
    <div className=" h-full flex flex-col items-center gap-2">
      ...
      <img
        className="h-4/5 w-auto z-10 absolute bottom-0 left-1/2 transform -translate-x-1/2"
        src={slide.pic}
        alt={slide.name}
      />
      </div>
     {isEgg &&  <button onClick={isEgg && showHatchModal} className="bg-yellow w-full text-white border-[3px] border-myBlack rounded-xl leading-none py-3"><span className="relative top-0.5">Hatch egg</span></button>}
    </div>
  );
}
